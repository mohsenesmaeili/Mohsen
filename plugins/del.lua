...
-----------------------------------------
local function delmsg (FullManager,Mohsen)
    msgs = FullManager.msgs 
    for k,v in pairs(Mohsen.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(Mohsen.messages_[0].chat_id_,{[0] = Mohsen.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(Mohsen.messages_[0].chat_id_, Mohsen.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
-----------------------------------------
